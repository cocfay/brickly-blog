...
class ConvertToWebP{
	...
	public function convert(
		...
		if(file_exists($this->fullPath)):
			...
			$this->extension = $ext;
			$this->newFilefullPath = str_replace('.'.$ext,'.'.$convertTo,$fullPath);
			//$this->newFilefullPath = $savePath;
			...
			else if($this->extension == 'webp' || $this->extension == 'WEBP') {
				$img = imagecreatefromwebp($this->fullPath);
				$isValidFormat = true;
			}
			...
			if($isValidFormat){
				...
				switch ($convertTo) {
					case 'webp':
						...
						break;
					case 'jpg':
						imagejpeg($img, $this->newFilefullPath,$this->outPutQuality);
						break;
