...
function observing(who, forWhat, callBack){
    ...
    /* Observing Usage */
    observing('div.container-items', 'div.item', function(mutation){
         console.log('Mutation:', $(mutation).find('input'))

         $('.item').each(function(e) {

            $(this).find('input, textarea').each(function(ip){
                console.log('input', ip);
                $(this).attr('name', $(this).attr('name').replace('[0]','['+e+']'));
                    console.log(e)
